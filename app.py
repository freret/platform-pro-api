...
if os.environ.get('DEPLOYMENT_ENV', "") == "HEROKU_PROD":
    app.config.update(
        ...
        CACHE_SECONDS=os.environ['CACHE_SECONDS'],
        THREADED=os.environ['THREADED'],
        MTA_KEY=os.environ['MTA_KEY'],
        STATIONS_FILE=os.environ['STATIONS_FILE'],
        CROSS_ORIGIN=os.environ['CROSS_ORIGIN']
    )
elif _SETTINGS_ENV_VAR in os.environ:
...
def index():
    return jsonify({
        'title': 'Platform Pro API',
        'readme': 'Visit https://github.com/jonthornton/MTAPI or https://github.com/freret/platform-pro-api for more info'
        })

@app.route("/alerts", methods=['GET'])
@cross_origin
def alerts():
    return mta.get_alerts()
