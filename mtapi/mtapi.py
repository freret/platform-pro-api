...
from mtaproto.feedresponse import FeedResponse, Trip, TripStop, TZ, AlertFeedResponse
...
class Mtapi(object):
    ...
    def __init__(self, key, stations_file, expires_seconds=60, max_trains=10, max_minutes=30, threaded=False):
        ...
        try:
            with open(stations_file, 'r') as f:
                ...
                self._stops_to_stations = self._build_stops_index(
                    self._stations)
    ...
    def _update(self):
        ...
        for i, feed_url in enumerate(self._FEED_URLS):
            ...
            max_time = self._last_update + \
                datetime.timedelta(minutes=self._MAX_MINUTES)
    ...
    def get_by_route(self, route):
        ...
        with self._read_lock:
            out = [self._stations[self._stops_to_stations[k]].serialize()
    ...
    def get_alerts(self):
        ...
        def isCurrent(alert, time):
            return any(time > int(p.get("start", 0)) and time < int(p.get("end", 9223372036854775807)) for p in alert.get("alert").get("activePeriod"))
        
        def formatAlert(d):
            d["alert"]["mercuryAlert"] = d["alert"].pop(
                "[transit_realtime.mercury_alert]", None)
            d["alert"]["informedRoutes"] = [x["routeId"] for x in d["alert"]["informedEntity"] if x.get("routeId", None) is not None]
            del d["alert"]["informedEntity"]
            d["alert"]["id"] = d["id"]
        ...
        with self._read_lock:    
            t = int(time.time())
            asList = list(map(MessageToDict, self._alerts.entity))
            currentAlerts = list(filter(lambda x: isCurrent(x, t), asList))
            alertsWithMercury = [x for x in currentAlerts if "[transit_realtime.mercury_alert]" in x.get("alert")]
            alertsList = map(formatAlert, alertsWithMercury)
